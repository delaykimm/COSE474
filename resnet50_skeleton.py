...
class ResNet50_layer4(nn.Module):
    def __init__(self, num_classes= 10 ): # Hint : How many classes in Cifar-10 dataset?
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size = 7, stride = 2 , padding = 3), #blank#, #blank#, #blank#, #blank#, #blank# ),
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(3, stride = 2, padding = 1) #kernel_size#, #stride#, #padding#)
        )
        self.layer2 = nn.Sequential(
            ResidualBlock(64, 64, 256, downsample = False),#in_channels, middle_channels, out_channels, downsample=False #blank#, #blank#, #blank#, #blank#),
            ResidualBlock(256, 64, 256, downsample = False), #(#blank#, #blank#, #blank#, #blank#),
            ResidualBlock(256, 64, 256, downsample = True) #(#blank#, #blank#,#blank#, #blank#)
        )
        ...
        self.fc = nn.Linear(1024, num_classes) #(input_features, out_features) #blank#, #blank#) # Hint : Think about the reason why fc layer is needed
